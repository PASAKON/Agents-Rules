...
| Claude transcripts (every machine: `$CLAUDE_CONFIG_DIR/projects/<slug>/<uuid>.jsonl`) | `BACKUP/MoonieX HQ/Claude-Transcripts/<machine>/<project-slug>/<uuid>.jsonl.gz` | CEO 2026-09-24: keep ALL, forever; off the box after 7 days; only via `prune_transcripts.py --archive`, never by hand; restore `--restore <uuid>`. Supersedes the old `Claude-Transcripts/<project>/` location (ADR 0031) |
| **Machine Contract family** — anything a machine keeps that is not in git: `$CLAUDE_CONFIG_DIR/uploads`, closed `Work/<task-id>/` output, Docker volumes `n8n_data` / `org-pgdata`, each machine's blueprint capture | `BACKUP/MoonieX HQ/{Claude-Uploads/<machine>/<date>.tar, Work-Archive/Agents-Work-<task>-<date>.tar, Docker-Volumes/<machine>/<volume>/<date>.tar|.sql.gz, Machine-Blueprints/<machine>/<date>/}` | CEO-approved 2026-09-24 ("ใส่ไว้ใน BACKUP/MoonieX HQ/{สิ่งที่จะเก็บ} ได้เลย เพิ่มนิยามให้ด้วย"); definitions in `gdrive-filing` § tree; which path goes where = Agents-Core `config/machine-contract.yaml` (ADR 0031, IRON §58). Tar/.gz + manifest, md5 read back from Drive BEFORE any local delete, one line in `~/.claude/logs/drive-archive.log`. Secrets NEVER here — another machine's HQ `Archive/` at 0600 |
