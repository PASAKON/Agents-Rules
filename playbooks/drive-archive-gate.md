...
| Claude transcripts (every machine: `$CLAUDE_CONFIG_DIR/projects/<slug>/<uuid>.jsonl`) | `BACKUP/MoonieX HQ/Claude-Transcripts/<machine>/<project-slug>/<YYYY-MM-DD>.tar.gz` + `.manifest.json` (one tar per slug per day; the sessions inside as `<uuid>.jsonl` + their sub-agent dirs) | CEO 2026-09-24: keep ALL, forever; off the box after 7 days; never by hand — Mac: `claude-home/tools/prune_transcripts.py --archive`; Contabo/winbox: `tools/drive_leg.py transcripts` through winbox's rclone (`scripts/rclone_via_winbox.sh`); restore one session `tools/drive_leg.py restore-transcript <uuid>` (Mac: `prune_transcripts.py --restore <uuid>`). Per-day packing because Contabo alone holds ~5,900 session files, most a few KB. First real run 2026-09-24 18:2x UTC from Contabo (md5 read back by id, local copies deleted after verify, one ledger line per day tar). The old `BACKUP/Claude-Transcripts/` folder (112 Mac tar.gz, 1.27 GiB) was MOVED whole on the CEO's yes 2026-09-24 into `MoonieX HQ/Claude-Transcripts/mac/` by one server-side DirMove (folder id 1FqUUq9CdwL7AvMnPzlui3kgUtKTMU4ZP unchanged) (ADR 0031) |
| **Machine Contract family** — anything a machine keeps that is not in git: `$CLAUDE_CONFIG_DIR/uploads`, closed `Work/<task-id>/` output, Docker volumes `n8n_data` / `org-pgdata`, each machine's blueprint capture | `BACKUP/MoonieX HQ/{Claude-Uploads/<machine>/<date>.tar, Work-Archive/Agents-Work-<task>-<date>.tar, Docker-Volumes/<machine>/<volume>/<date>.tar|.sql.gz, Machine-Blueprints/<machine>/<date>/, State-DB/<machine>/tasks-<date>.sqlite.gz}` | CEO-approved 2026-09-24 ("ใส่ไว้ใน BACKUP/MoonieX HQ/{สิ่งที่จะเก็บ} ได้เลย เพิ่มนิยามให้ด้วย"); definitions in `gdrive-filing` § tree; which path goes where = Agents-Core `config/machine-contract.yaml` (ADR 0031, IRON §58). Tar/.gz + manifest, md5 read back from Drive BEFORE any local delete, one line in `~/.claude/logs/drive-archive.log`. Secrets NEVER here — another machine's HQ `Archive/` at 0600. **First objects 2026-09-24 (Contabo, `tools/drive_leg.py`):** `Machine-Blueprints/contabo/2026-09-24/contabo-blueprint-2026-09-24.tar` id `1ahZQMvGnKzh1xtyOTT4bp-d5wwVrWpi-` (25 files, md5 `45ca1274…`) · `Machine-Blueprints/winbox/2026-09-24/winbox-blueprint-2026-09-24.tar` `1H6fj6l77C7bJC3mkTV4HlpVU1L70CqUp` (67 files, md5 `591593f3…`) · `Claude-Uploads/contabo/2026-09-24.tar` `1C6q1Lz9l3XvDRvwLrEWXInBQgcKxwzs-` (14 files, 11.5 MB, md5 `c83e41c8…`) · `Docker-Volumes/contabo/n8n_data/2026-09-24.tar` `1vvQ9NAJ81giQJwxBNQ_AhF8ElG1EjsGl` (1,548,592,128 B, md5 `5ab511a9…`, the first backup of n8n's data ever; the n8n `config` file with the encryptionKey is EXCLUDED — it is a secret and lives in the secrets bundle) · `org-pgdata` as `pg_dumpall -U org` `.sql.gz` (role `org`, not `postgres`). Sources stay on the box for these four kinds (git / live volume / uploads); only transcripts are deleted after verify. **State-DB** (CEO "อนุมัติ" 2026-09-24, id `15xudjmiHK6MfSCxM48cqwjPOKOWgoNzO`): `state/tasks.db` via sqlite's online backup → `contabo/tasks-<date>.sqlite.gz` weekly (`tools/drive_leg.py state-db`) |
...
| `contabo: /root/mooniex-agents-pre*-20260807-*.tgz` — two self-backups of `/opt/mooniex-agents` taken 2026-08-07 before its git swap/sync (code + `.venv` + `mcp/`; not provably all on GitHub) | `BACKUP/Contabo-mooniex-agents-pre-20260807.tar` + `.manifest.json` at the BACKUP root (same precedent as `Agents-worktrees-*`) | CEO-approved in chat 2026-09-23 ("ลบได้ถ้า… ถ้าไม่ชัวร์ สำรองก่อน", answering the request that named both files). scp'd to the Mac, md5 checked against the box, one outer tar + MANIFEST.json (sha256/md5 per item, restore line), Drive REST resumable upload via `tools/work_archive.py`'s uploader, `md5Checksum` read back by file id BEFORE the box copies were deleted; one line in `~/.claude/logs/drive-archive.log`. |
| `winbox:` everything that is not Cookie Run, before the 2026-09-24 Windows reinstall — code not on GitHub, Downloads leftovers, `C:\mooniex` data folders, Desktop/Pictures | `BACKUP/Winbox Reinstall 2026-09-24/<group>.tar` + `.manifest.json` | CEO-approved in chat 2026-09-24 ("อณุมัติ", answering a request that named this folder). Streamed from winbox (`tar` → `rclone rcat`, no local temp), with md5 hashed on the way and compared with Drive's `md5Checksum` before anything is deleted. Files already on Drive (md5 match against the YT: ILAG index) were deleted without re-upload; one line each in `~/.claude/logs/drive-archive.log`. Cookie Run data goes to `BACKUP/CookieRun Backup/{play_rec,bot_sessions,jumpsweeps,modelplay,playsets,box-extras}/` (same approval, Cookie Run CTO). |
| `mac:` `~/.claude` (Claude Code sessions) and `~/MoonieXHQ` code-not-on-a-remote + gitignored data, before the 2026-09-24 Mac wipe | `BACKUP/Mac-Reinstall-2026-09-24-{claude-home,MoonieXHQ-code,MoonieXHQ-data}.tar` + `.manifest.json` at the BACKUP root (Agents-worktrees / Contabo precedent) | CEO in chat 2026-09-24 ("session ของ Claude และ MoonXHQ … ของ MAC สำรองให้เลย"). `scripts/stream_backup_to_drive.py` (freeze → tar → upload → size+md5 read back by id → manifest); route = winbox's rclone over ssh, or `tools/work_archive.py` REST when winbox is down. Thin git bundles (prerequisites on remotes), stash + dirty patches, SQLite via `.backup`. **Secrets never go to Drive**: `.env*`, certs, `~/.ssh`, `~/.config/mooniex`, `~/.claude.json`, launchd plists, shell history → `contabo:/opt/MoonieXHQ/Archive/mac-secrets-<date>/` (root 0600, sha256 both ends). Nothing deleted locally. |
| `mac:` `~/Library/Application Support/CloudDocs/session/i` — the orphaned iCloud Drive store (not in client.db, invisible in Finder), before the Mac wipe | `BACKUP/Mac-Reinstall-2026-09-25-iCloudLeftovers-p01…p08.tar` + `.manifest.json`, and `…-p09-f786.mov` (one 7.0 GB file uploaded raw) at the BACKUP root | CEO in chat 2026-09-25 ("สำรองด้วย ระหว่างสำรองลบ ข้อมูลที่ยืนยันได้ว่าสำรองแล้ว ออกให้เลยเป็นระยะ ๆ"). Only files whose md5 was not already on Drive were uploaded; size+md5 read back by id per part. Media proven by md5 moved to `~/.Trash` (the CEO emptied it); **documents backed up but never removed** — the CEO reviews them himself. Ids in `~/.claude/logs/drive-archive.log`. |
