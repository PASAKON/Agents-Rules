...
- **Date:** 2026-09-18
- **Status:** Accepted (CEO ruling 2026-09-17 #5, memory `project_org_session_architecture_2026_09`) — **implementation in progress**, see status table
...
(910 → 946 tasks, 137 sessions, 7.5k events) and Contabo's (11 tasks, all terminal, all
...
4. **Cutover order:** Mac first (freeze watchdog → migrate → flip `ORG_DB_URL` in the
   MCP configs, launchd plists and `cto-claude.sh` → verify → archive `state/tasks.db`),
   Contabo last (only after its two live CTO sessions are restarted: pull tree, import
   its 11 rows, flip). Live sessions keep their old backend until restarted, so their
   delta is imported afterwards. Nothing is deleted before it has been read (ADR 0024).
...
| W1b case-insensitive id prefixes on both backends | merged | main b46dbfae |
| Real-data migration rehearsal on a Mac-local Postgres | failed → fix in flight | found: no target schema, swallowed error aborts the transaction, `events` identity never advanced; task-78586938 |
| pytest inherits the C-level session env (5 red on main) | fix in flight | task-78586938 (root `conftest.py`) |
| Postgres on Contabo | **waiting for the CEO** | the CTO session's classifier refuses remote shell writes; script ready for `! bash` |
| Migrate Mac data + Mac cutover | not started | needs the two rows above |
| Contabo import + flip | **waiting for the CEO** | CTO sessions 6ebacd0e / e1e3d3ef live on that tree |
| Wiki changelog `projects/mooniex-agents.md` | at close | — |
